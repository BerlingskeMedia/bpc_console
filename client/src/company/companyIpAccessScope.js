...
export default class CompanyIpAccessScope extends React.Component {
  ...
  render() {
    ...
    const ipAccessScopes = data.map((rule) => <IpAccessScopeItem key={rule.ip} data={rule}
                    ...
                    <div className={inputClassName}>
                      ...
                      >
                        ...
                        {scopes.map((scope, index) => {
                          return <option key={index} value={scope}>{scope}</option>;
                        })}
                      </select>
                    </div>
                  </td>
                  <td style={{textAlign: 'right'}}>
                    <button type="button" className='btn btn-xs btn-success' onClick={this.addIpScope}
                            disabled={!this.state.inputValid || this.state.addingItem} style={{minWidth: '90px'}}>
