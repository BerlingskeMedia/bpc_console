...
export default class CompanyUsers extends React.Component {
  ...
  onChangeAddUser() {
    ...
    if (value.length > 0) {
      ...
      this.setState({hasInput: true, inputAddUserValid: false, inputCreateUserValid}, () => {
        ...
        this.setState({
          searchUserInProgress: true, searchUserTimer: setTimeout(() => {
            this.searchUser(value)
                .then(foundUser => this.setState({
                  inputAddUserValid: foundUser !== null,
                  foundUser,
                  searchUserInProgress: false
                }));
          }, 1000)
  ...
  render() {
    ...
    let items = users.map((user) => <CompanyUser key={user.uid} user={user} removeUser={this.props.removeUser}
    ...
    if (showTruncatedUserList) {
      items.push(
          <tr key={-20}>
            <td>
              <div>Showing {maxUsersToShow} out of {originalUserCount} users. <small>Use input below to find and remove
                a user.</small></div>
              <div></div>
            </td>
            <td></td>
    ...
    items.push(
        <tr key={-1}>
          ...
          <td style={{textAlign: 'right'}}>
            <button type="button" className='btn btn-xs btn-default' onClick={this.createUser}
                    disabled={createButtonDisabled} style={{minWidth: '90px'}}>
              <span className='glyphicon glyphicon-cloud-upload' aria-hidden="true"></span> <span>Create</span>
            </button>
            <span>&nbsp;</span>
            {foundUserIsAlreadyAdded
                ? <button type="button" className='btn btn-xs btn-danger'
                          onClick={this.removeUser.bind(this, this.state.foundUser)} style={{minWidth: '90px'}}>
